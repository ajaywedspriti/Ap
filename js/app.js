...
const comment = (() => {
    ...
    const send = async () => {
        ...
        if (nama.length >= 35) {
            alert('Name should not be more then 35 characters for now..!!');
            return;
        }

        if (hadir == 0) {
            alert('Please selecte your availability..!!');
            return;
        }
        ...
        // let dataset = [];
        dataset.push(data);
        localStorage.setItem('resdata', JSON.stringify(dataset));
        setTimeout(() => {
            resetForm();
            ucapan();
    ...
    const ucapan = async () => {
        ...
        //         if (res.code == 200) {
                    ...
                    resdata.forEach((data) => UCAPAN.appendChild(renderCard(data)));
